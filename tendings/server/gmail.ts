...
export async function gmailThreads(request: RequestLike, response: ResponseLike) {
  try {
    ...
    const { data, error } = await db(config).from("tending_gmail_threads").select("gmail_thread_id, sender, subject, snippet, latest_message_at, unread, reply_worthy, source_url, importance_score, urgency, priority_person").eq("owner_id", user.id).eq("unread", true).eq("suppressed", false).order("importance_score", { ascending: false }).order("latest_message_at", { ascending: false }).limit(30);
