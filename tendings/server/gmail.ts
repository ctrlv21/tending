...
function analysisPreview(value: string) { return value.replace(/\s+/g, " ").replace(/<[^>]*>/g, " ").trim().slice(0, 900); }
...
function hasAdministrativeSignal(subject: string, snippet: string) { return /\b(payment|invoice|statement|balance|rent|lease|bill(?:ing)?|tax|refund|renewal|subscription|appointment|meeting|rescheduled|cancelled|interview|sponsorship|partnership|proposal|contract|account|security|verification|shipment|delivery|reservation|travel|application)\b/i.test(`${subject} ${snippet}`); }
function looksReplyWorthy(subject: string, snippet: string) { return /\?|\b(can you|could you|would you|please|let me know|thoughts\?|confirm|urgent|asap|deadline|today|tomorrow|time-sensitive|important|need your|action required)\b/i.test(`${subject} ${snippet}`) || hasAdministrativeSignal(subject, snippet); }
...
function urgencyScore(subject: string, snippet: string, unread: boolean, priorityPerson: boolean, matchesKeyword: boolean, latestAt: number) { const text = `${subject} ${snippet}`; let score = unread ? 1 : 0; if (priorityPerson) score += 4; if (matchesKeyword) score += 3; if (/\b(urgent|asap|action required|time-sensitive|eod|end of day)\b/i.test(text)) score += 5; if (/\b(payment|invoice|statement|balance|rent|lease|bill(?:ing)?|tax|refund|renewal|subscription|appointment|meeting|rescheduled|cancelled|interview|sponsorship|partnership|proposal|contract|account|security|verification|shipment|delivery|reservation|travel|application)\b/i.test(text)) score += 5; if (/\b(deadline|due|today|tomorrow|by friday|this week)\b/i.test(text)) score += 3; if (/\?|\b(can you|could you|would you|please|confirm|let me know)\b/i.test(text)) score += 2; if (Date.now() - latestAt > 86_400_000) score += 1; return score; }
...
async function syncGmailConnection(ownerId: string, config: GmailConfig, connection: GmailConnectionRow) {
  ...
  // AI can suppress only an already non-actionable, non-administrative item.
  // Hard Gmail spam/promotion filters still run before any excerpt is analysed.
  const enriched = usable.map(({ _inbound, _latestAt, _analysisText, ...thread }) => { const analysis = analyses.get(thread.gmail_thread_id); const actionSignal = hasAdministrativeSignal(thread.subject, _analysisText) || thread.reply_worthy || thread.priority_person || thread.keyword_match; const ignored = analysis?.urgency === "ignore" && !actionSignal; const score = Math.max(thread.importance_score, analysis?.score ?? 0); const urgency = analysis?.urgency === "urgent" ? "urgent" : analysis?.urgency === "reply" ? "reply" : score >= 7 ? "urgent" : score >= 3 ? "reply" : "watch"; return { ...thread, suppressed: thread.suppressed || ignored, reply_worthy: !ignored && _inbound && thread.unread && (thread.reply_worthy || analysis?.urgency === "urgent" || analysis?.urgency === "reply"), importance_score: ignored ? 0 : score, urgency, analysis_reason: analysis?.reason ?? null, analysis_provider: analysis ? "anthropic" : null, analyzed_at: analysis ? new Date().toISOString() : null }; });
...
export async function gmailSenders(request: RequestLike, response: ResponseLike) {
  try {
    ...
    const connectedAddresses = new Set(connections.map((connection) => connection.gmail_email?.toLowerCase()).filter((email): email is string => Boolean(email)));
    ...
    const usableContact = (item: { identifier: string; label: string }) => !connectedAddresses.has(item.identifier) && !isAutomatedAddress(item.identifier) && matchesSearch(item);
