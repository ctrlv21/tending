...
export async function gmailSenders(request: RequestLike, response: ResponseLike) {
  try {
    const config = getConfig(); const user = await getAuthenticatedUser(request, config); const connection = await getConnection(user.id, config); const query = String(first(request.query?.query) ?? "").trim().replace(/[^\p{L}\p{N}@._\-\s]/gu, "").slice(0, 80);
    if (!connection || connection.status !== "connected") throw new Error("Connect Gmail to search recent senders.");
    ...
    const listing = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?${new URLSearchParams({ q: `from:${query}`, maxResults: "20" })}`, { headers: { Authorization: `Bearer ${accessToken}` } });
    const payload = await listing.json() as { messages?: Array<{ id: string }>; error?: { message?: string } };
    if (!listing.ok) throw new Error(payload.error?.message || "Gmail senders could not be searched.");
    const messages = await Promise.all((payload.messages ?? []).map(async (item) => { const itemResponse = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${encodeURIComponent(item.id)}?format=metadata&metadataHeaders=From`, { headers: { Authorization: `Bearer ${accessToken}` } }); if (!itemResponse.ok) return null; const message = await itemResponse.json() as { payload?: { headers?: Array<{ name?: string; value?: string }> } }; const from = header(message.payload?.headers, "From"); const email = senderEmail(from); return email ? { identifier: email, label: senderName(from) } : null; }));
    const seen = new Set<string>(); const suggestions = messages.filter((item): item is NonNullable<typeof item> => Boolean(item)).filter((item) => !seen.has(item.identifier) && Boolean(seen.add(item.identifier))).slice(0, 6);
...
export async function gmailThreads(request: RequestLike, response: ResponseLike) {
  try {
    ...
    const { data, error } = await db(config).from("tending_gmail_threads").select("gmail_thread_id, sender, subject, snippet, latest_message_at, unread, reply_worthy, source_url, importance_score, urgency, priority_person, keyword_match").eq("owner_id", user.id).eq("unread", true).eq("suppressed", false).order("importance_score", { ascending: false }).order("latest_message_at", { ascending: false }).limit(30);
    if (error) throw error;
