...
async function syncGmailConnection(ownerId: string, config: GmailConfig, connection: GmailConnectionRow) {
  ...
  const enriched = usable.map(({ _inbound, _latestAt, ...thread }) => { const analysis = analyses.get(thread.gmail_thread_id); const ignored = analysis?.urgency === "ignore"; const score = Math.max(thread.importance_score, analysis?.score ?? 0); const urgency = ignored ? "watch" : analysis?.urgency === "urgent" ? "urgent" : analysis?.urgency === "reply" ? "reply" : score >= 7 ? "urgent" : score >= 3 ? "reply" : "watch"; return { ...thread, suppressed: thread.suppressed || ignored, reply_worthy: !ignored && _inbound && thread.unread && (thread.reply_worthy || analysis?.urgency === "urgent" || analysis?.urgency === "reply"), importance_score: ignored ? 0 : score, urgency, analysis_reason: analysis?.reason ?? null, analysis_provider: analysis ? "anthropic" : null, analyzed_at: analysis ? new Date().toISOString() : null }; });
  const { error: resetError } = await db(config).from("tending_gmail_threads").update({ suppressed: true, reply_worthy: false, updated_at: new Date().toISOString() }).eq("owner_id", ownerId).eq("gmail_connection_id", connection.id); if (resetError) throw resetError; if (enriched.length) { const { error } = await db(config).from("tending_gmail_threads").upsert(enriched, { onConflict: "owner_id,gmail_connection_id,gmail_thread_id" }); if (error) throw error; }
