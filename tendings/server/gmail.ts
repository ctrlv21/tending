...
function looksReplyWorthy(subject: string, snippet: string) { return /\?|\b(can you|could you|would you|please|let me know|thoughts\?|confirm|urgent|asap|deadline|today|tomorrow|time-sensitive|important|need your|action required|rent|lease|payment due|past due|invoice|bill|balance due)\b/i.test(`${subject} ${snippet}`); }
function looksPromotional(from: string, labels: string[] | undefined, subject: string, snippet: string) { const text = `${subject} ${snippet}`; const email = senderEmail(from); const identity = `${from} ${email}`; const criticalAdmin = /\b(rent|lease|payment due|past due|invoice|bill(?:ing)?|balance due|account (?:alert|suspended)|security alert|verification|appointment|reservation)\b/i.test(text); if (labels?.some((label) => ["SPAM", "TRASH", "CATEGORY_PROMOTIONS", "CATEGORY_SOCIAL", "CATEGORY_FORUMS"].includes(label))) return true; if (criticalAdmin) return false; return Boolean(labels?.includes("CATEGORY_UPDATES")) || /(?:^|\b)(?:no-?reply|newsletter|notifications?|marketing|updates)@/i.test(email) || /\b(?:substack|aritzia)\b/i.test(identity) || /\b(unsubscribe|manage preferences|view (?:this )?in browser|weekly digest|new post from|recommended for you|shop now|free shipping|sale ends|airdrop|crypto|forex|telegram|whatsapp|make money|investment opportunity|paid partnership|brand deal)\b/i.test(text); }
function keywordMatch(text: string, words: string[]) { const lower = text.toLowerCase(); return words.some((word) => lower.includes(word)); }
function urgencyScore(subject: string, snippet: string, unread: boolean, priorityPerson: boolean, matchesKeyword: boolean, latestAt: number) { const text = `${subject} ${snippet}`; let score = unread ? 1 : 0; if (priorityPerson) score += 4; if (matchesKeyword) score += 3; if (/\b(urgent|asap|action required|time-sensitive|eod|end of day)\b/i.test(text)) score += 5; if (/\b(rent|lease|payment due|past due|invoice|bill(?:ing)?|balance due)\b/i.test(text)) score += 7; if (/\b(deadline|due|today|tomorrow|by friday|this week)\b/i.test(text)) score += 3; if (/\?|\b(can you|could you|would you|please|confirm|let me know)\b/i.test(text)) score += 2; if (Date.now() - latestAt > 86_400_000) score += 1; return score; }
...
async function syncGmailConnection(ownerId: string, config: GmailConfig, connection: GmailConnectionRow) {
  ...
  const threads = await Promise.all((listing.threads ?? []).slice(0, 50).map(async ({ id }) => { const response = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/threads/${encodeURIComponent(id)}?format=metadata&metadataHeaders=From&metadataHeaders=Subject`, { headers: { Authorization: `Bearer ${accessToken}` } }); if (!response.ok) return null; const thread = await response.json() as { id: string; snippet?: string; messages?: Array<{ internalDate?: string; labelIds?: string[]; payload?: { headers?: Array<{ name?: string; value?: string }> } }> }; const latest = thread.messages?.[thread.messages.length - 1]; if (!latest) return null; const subject = header(latest.payload?.headers, "Subject") || "No subject"; const snippet = preview(thread.snippet ?? ""); const unread = Boolean(latest.labelIds?.includes("UNREAD")); const fromValue = header(latest.payload?.headers, "From"); const sender = senderName(fromValue); const email = senderEmail(fromValue); const latestAt = Number(latest.internalDate ?? Date.now()); const suppressed = looksPromotional(fromValue, latest.labelIds, subject, snippet); const priorityPerson = priorities.has(sender.toLowerCase()) || priorities.has(email); const matchesKeyword = keywordMatch(`${subject} ${snippet}`, keywords); const score = suppressed ? 0 : urgencyScore(subject, snippet, unread, priorityPerson, matchesKeyword, latestAt); const inbound = !connection.gmail_email || !fromValue.toLowerCase().includes(connection.gmail_email.toLowerCase()); return { owner_id: ownerId, gmail_connection_id: connection.id, gmail_thread_id: thread.id, sender, sender_email: email || null, subject, snippet, latest_message_at: new Date(latestAt).toISOString(), unread, reply_worthy: inbound && unread && !suppressed && (looksReplyWorthy(subject, snippet) || priorityPerson || matchesKeyword), source_url: `https://mail.google.com/mail/u/${encodeURIComponent(connection.gmail_email ?? "0")}/#all/${thread.id}`, updated_at: new Date().toISOString(), importance_score: score, urgency: score >= 7 ? "urgent" : score >= 3 ? "reply" : "watch", priority_person: priorityPerson, keyword_match: matchesKeyword, suppressed, _inbound: inbound, _latestAt: latestAt }; }));
  const usable = threads.filter((thread): thread is NonNullable<typeof thread> => Boolean(thread));
  const analyses = await analyzeMessages(usable.filter((thread) => thread._inbound && thread.unread && !thread.suppressed).sort((a, b) => b.importance_score - a.importance_score).map((thread) => ({ id: thread.gmail_thread_id, source: "gmail" as const, text: `${thread.subject}\n${thread.snippet}`, ageHours: Math.max(0, Math.round((Date.now() - thread._latestAt) / 3_600_000)), priorityPerson: thread.priority_person, watchWord: thread.keyword_match })));
  // AI ranks a message; it never gets to suppress it. Promotion/spam suppression
  // remains entirely deterministic and is evaluated before an excerpt is sent.
  const enriched = usable.map(({ _inbound, _latestAt, ...thread }) => { const analysis = analyses.get(thread.gmail_thread_id); const score = Math.max(thread.importance_score, analysis?.score ?? 0); const urgency = analysis?.urgency === "urgent" ? "urgent" : analysis?.urgency === "reply" ? "reply" : score >= 7 ? "urgent" : score >= 3 ? "reply" : "watch"; return { ...thread, reply_worthy: _inbound && thread.unread && (thread.reply_worthy || analysis?.urgency === "urgent" || analysis?.urgency === "reply"), importance_score: score, urgency, analysis_reason: analysis?.reason ?? null, analysis_provider: analysis ? "anthropic" : null, analyzed_at: analysis ? new Date().toISOString() : null }; });
...
export async function gmailSenders(request: RequestLike, response: ResponseLike) {
  try {
    ...
    const lookupMessage = !peopleResponse.ok ? "Contact lookup needs the Google People API enabled and Gmail reconnected with Contacts permission." : suggestions.length ? null : "No matching contact found. Try their full name or email address.";
    return response.status(200).json({ suggestions, lookupMessage });
...
export async function gmailThreads(request: RequestLike, response: ResponseLike) {
  try {
    ...
    const { data, error } = await db(config).from("tending_gmail_threads").select("gmail_thread_id, sender, subject, snippet, latest_message_at, unread, reply_worthy, source_url, importance_score, urgency, priority_person, keyword_match, analysis_reason").eq("owner_id", user.id).eq("unread", true).eq("suppressed", false).order("importance_score", { ascending: false }).order("latest_message_at", { ascending: false }).limit(30);
