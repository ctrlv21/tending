...
function looksPromotional(from: string, labels: string[] | undefined, subject: string, snippet: string) { const text = `${subject} ${snippet}`; const email = senderEmail(from); return Boolean(labels?.some((label) => ["CATEGORY_PROMOTIONS", "CATEGORY_SOCIAL", "CATEGORY_FORUMS", "CATEGORY_UPDATES"].includes(label))) || /(?:^|\b)(?:no-?reply|newsletter|notifications?|marketing|updates)@/i.test(email) || /(?:^|\.)(?:substack\.com|aritzia\.com)$/i.test(email) || /\b(unsubscribe|manage preferences|view (?:this )?in browser|weekly digest|new post from|recommended for you|shop now|free shipping|sale ends)\b/i.test(text); }
