...
export async function gmailCallback(request: RequestLike, response: ResponseLike) {
  const code = first(request.query?.code); const state = first(request.query?.state); const providerError = first(request.query?.error); if (providerError || !code || !state) return response.redirect(302, `${appUrl(request)}/tending?gmail=cancelled`);
  try {
    ...
    await syncGmail(ownerId, config); return response.redirect(302, `${appUrl(request)}/tending?gmail=connected`);
  } catch (error) { return response.redirect(302, `${appUrl(request)}/tending?gmail=error&message=${encodeURIComponent(error instanceof Error ? error.message : "Gmail connection failed.")}`); }
}
