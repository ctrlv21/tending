...
export async function xTestFeed(request: RequestLike, response: ResponseLike) { try { const config = getConfig(); const user = await currentUser(request, config); const item = await connection(user.id, config); if (!item || item.status !== "connected") throw new Error("Connect X before testing its API feed."); const token = await accessToken(item, config); const query = new URLSearchParams({ max_results: "100", "dm_event.fields": "created_at,sender_id,text,dm_conversation_id" }); const apiResponse = await fetch(`https://api.x.com/2/dm_events?${query}`, { headers: { Authorization: `Bearer ${token}` } }); const payload = await apiResponse.json() as { data?: XEvent[]; errors?: Array<{ detail?: string; title?: string }> }; const timestamps = (payload.data ?? []).map((event) => event.created_at).filter((value): value is string => Boolean(value)).sort(); response.setHeader("Cache-Control", "no-store"); return response.status(apiResponse.ok ? 200 : 400).json({ ok: apiResponse.ok, status: apiResponse.status, eventCount: payload.data?.length ?? 0, newestEventAt: timestamps[timestamps.length - 1] ?? null, requestId: apiResponse.headers.get("x-request-id") || apiResponse.headers.get("x-transaction-id"), error: apiResponse.ok ? null : payload.errors?.[0]?.detail || payload.errors?.[0]?.title || "X rejected the test request." }); } catch (error) { return response.status(400).json({ ok: false, error: error instanceof Error ? error.message : "X API test could not run." }); } }
